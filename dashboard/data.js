...
//
// Synced against: v4.7 (SAVE_VERSION 20), Aug 29 2026 — v4.5/4.6 story text
// unchanged; v4.7 added dean_budget, lean_away, commit_swing, growth_spurt,
// the janitor supply closet, the blank-check split (pledge/warchest), and
// swapped the box-score mapping (FRC→rebounds, ATH→steals).
// ============================================================================
...
const STORIES = [
  ...
  // ================= SYSTEM DIALOGS =================
  { id: 'supply', fam: 'plumbing', name: 'THE SUPPLY CLOSET', who: 'coach', trigger: '50%/week: THE JANITOR wheels his cart past your door and sets a small item on your desk without a word. The mop keeps dripping. He waits.', beats: [ { key: 'start', setup: 'TAKE IT (into the bag; a full bag = a kid outside the arena walks off with it, delighted) or LEAVE IT ON THE DESK (the janitor shrugs, pockets it, and re-donates it to the mystery it came from). His figure: the supply closet, shelves, a swinging bulb, the mop and bucket.' } ] },
  ...
  // ================= v4.7 ADDITIONS (Aug 29) =================
  { id: 'dean_budget', fam: 'regulars', name: 'THE WEEKLY BUDGET', who: 'coach', trigger: 'EVERY home week opens with it, before anything else knocks (tournament weeks skip her — you\'re on the road).', beats: [ { key: 'start', setup: 'The dean at your door with the week\'s envelope: the stipend amount plus a snark line keyed to your heat (≥60 severance jokes · ≥30 auditor smiles · trophies = warm-ish · 0¢ in the aging seasons = "compensated in experience"). Tap-through; the credits already landed.' } ] },
  { id: 'lean_away', fam: 'plumbing', name: 'THE COLD SHOULDER (a lean-away is a story)', who: 'coach', trigger: 'After a recruiting action, for EACH name that soured on the risk roll.', beats: [ { key: 'start', setup: 'Word comes back: [name] has gone cold — his holo-agent is "exploring the galaxy\'s options."', choices: [ { label: 'CALL HIM PERSONALLY (1¢)', cost: 1, up: '50% SPIRIT', down: '10% DRAMA (it can get worse)', out: { up: 'Twenty minutes about his jumper and his grandmother\'s cooking. (+12%)', mid: 'Polite. Fine. The freeze stops spreading. (+5%)', down: 'You call mid-dinner — at the RIVAL\'s recruiting dinner. NO CONTACT 2w, −10%.' } }, { label: 'GIVE HIM SPACE', up: '10% SPIRIT', down: '25% DRAMA (cold can freeze)', out: { up: 'The silence reads as confidence. (+8%)', mid: 'He keeps the space. The board holds its breath.', down: 'The rival sends a highlight reel with his name in gold letters. (−8%)' } } ] } ] },
  { id: 'commit_swing', fam: 'weekly', name: 'THE RECRUITING TRAIL (commitment weather)', who: 'coach', weight: 2, when: 'Any unsigned board name with commitment or digits.', trigger: 'Weekly roll: a random unsigned name swings ±6–14% on its own — a cousin got into astro-engineering, a rival campus scarf photo, a bar-stream repost of your last dunk.', beats: [ { key: 'start', setup: 'One flavor line (3 up / 3 down), then [name]: COMMITMENT ±N.' } ] },
  { id: 'growth_spurt', fam: 'weekly', name: 'THE GROWTH SPURT', who: 'player', weight: 1, trigger: 'Weekly roll (rare): a body rewrites itself overnight — small kids grow OUT OF THIS WORLD (+24–37cm, +FRC), tall ones can shrink (−24–37cm, +ATH, −mood). The columns read him differently after; his listed position may retrain in the following weeks.', beats: [ { key: 'start', setup: 'The team doctor measures him twice, then measures the chart. Jersey, shoes, position: all suddenly negotiable.' } ] },
];
...
const ITEMS = [
  ...
  { id: 'check', name: "BOOSTER'S BLANK CHECK", short: 'CHECK', rarity: 'rare', context: ['recruiting'], target: 'prospect', flavor: 'The amount is blank. The strings are not.', effect: 'the recruit you DROP it on SIGNS. Today: greyed on the board, unremovable, skipped by decay/recruiting/letters — auto-commits on signing day ("signed weeks ago")', up: '2% LOOT', down: '25% SCANDAL', out: { mid: 'He signs on the spot. Done. Sealed. Nothing wanes, nothing waits for signing day.', down: 'So does the check — into a league evidence folder. THE SUBPOENA lands in 4 weeks. (+20 heat)' } },
  { id: 'pledge', name: "BOOSTER'S PLEDGE", short: 'PLEDG', rarity: 'common', context: ['recruiting'], target: 'prospect', flavor: 'A handshake, a promise, a hat with your logo already on it.', effect: 'one recruit: commitment +20% — a CHUNK, not a signature; chunks can wane', up: '5% SPIRIT', down: '5% DRAMA', out: { up: 'He wears the hat to school the next day. THE HAT IS WORKING. (+28%)', mid: 'He shakes on it and keeps the hat. (+20%)', down: 'He posts the hat on a resale stream an hour later. (+8%)' } },
  { id: 'warchest', name: 'THE WAR CHEST', short: 'CHEST', rarity: 'rare', context: ['recruiting'], target: 'prospect', flavor: 'A crate of "recruiting materials". It clinks.', effect: 'one recruit: commitment +40% — a big chunk, and chunks can wane', up: '5% SPIRIT', down: '10% SCANDAL', out: { up: 'His whole holo-feed is your program for a week. (+50%)', mid: 'He leans in hard. (+40%)', down: 'A league auditor counted the crates on the loading dock. (+40%, +8 heat)' } },
